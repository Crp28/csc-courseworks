...
def process_event_history(log: dict[str, list[dict]],
    ...
    # initial month -- month of the first record
    new_month(customer_list, billing_date.month, billing_date.year)

    for event_data in log['events']:
        src = event_data.get('src_number')
        dst = event_data.get('dst_number')
        src_customer = find_customer_by_number(src, customer_list)
        dst_customer = find_customer_by_number(dst, customer_list)
        time = datetime.datetime.strptime(event_data.get('time'),
                                          "%Y-%m-%d %H:%M:%S")
        # advance time for all users if event comes from a new month
        if time.month != billing_month:
            billing_month = time.month
            new_month(customer_list, time.month, time.year)
        duration = event_data.get('duration')
        src_loc = (event_data.get('src_loc')[0], event_data.get('src_loc')[1])
        dst_loc = (event_data.get('dst_loc')[0], event_data.get('dst_loc')[1])
        # makes new Call object and record the Call in various places
        if event_data.get('type') == 'call':
            new_call = Call(src, dst, time, duration, src_loc, dst_loc)
