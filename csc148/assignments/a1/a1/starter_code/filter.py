        ...
        try:
            for cust in customers:
                if cust.get_id() == int(filter_string):
                    temp = cust.get_history()
        except:
            return data
        if temp is None:
            return data
        for call in data:
            if call in temp[0] or call in temp[1]:
    ...
    def apply(self, customers: list[Customer],
        ...
        if filter_string[0] == "L" and len(filter_string) <= 4:
            try:
                ...
                return data
        elif filter_string[0] == "G" and len(filter_string) <= 4:
            try:
                duration = int(filter_string[1:4])
                if duration < 0:
                    return data
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        """
        cord_list = filter_string.split(", ")
        temp_list = []
        if len(cord_list) != 4:
            return data
        try:
            lowerlong = float(cord_list[0])
            lowerlat = float(cord_list[1])
            upperlong = float(cord_list[2])
            upperlat = float(cord_list[3])
            for call in data:
                src_long = call.src_loc[0]
                src_lat = call.src_loc[1]
                dst_long = call.dst_loc[0]
                dst_lat = call.dst_loc[1]
                if lowerlong <= src_long <= upperlong and lowerlat <= src_lat <= upperlat:
                    temp_list.append(call)
                elif lowerlong <= dst_long <= upperlong and lowerlat <= dst_lat <= upperlat:
                    temp_list.append(call)
            return temp_list
        except:
            print("error")
