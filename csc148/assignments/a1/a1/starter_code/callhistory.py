...
class CallHistory:
    ...
    def register_outgoing_call(self, call: Call) -> None:
        ...
        """
        if (int(call.time.month), int(call.time.year)) not in self.outgoing_calls.keys():
            self.outgoing_calls[(int(call.time.month), int(call.time.year))] = [call]
        else:
            self.outgoing_calls[(int(call.time.month), int(call.time.year))] += [call]
    ...
    def register_incoming_call(self, call: Call) -> None:
        ...
        """
        if (int(call.time.month), int(call.time.year)) not in self.incoming_calls.keys():
            self.incoming_calls[(int(call.time.month), int(call.time.year))] = [call]
        else:
            self.incoming_calls[(int(call.time.month), int(call.time.year))] += [call]
